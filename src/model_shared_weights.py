...
class LearnedWaveSim(object):
    ...
    def update_devito(self, velIndex=1):
        ...
        t0 = 0.0
        nt = int(1 + (tn - t0) / dt)
        self.virt_timestep = int(nt // self.virtSteps)
    ...
    def _build_model(self):
        ...
        self.fake_A_sample = tf.placeholder(tf.float32, [None, self.image_size0, self.image_size1, 
    ...
    def train(self, args):
        ...
        datasetSize = (len(velIndex)*len(batch_idxs)*self.virtSteps, 1, self.image_size0, \
            ...
            for isrc in range(0, len(batch_idxs)):
                ...
                for time_index in range(self.virtSteps):
                    Noisy_wave[tempTrainingData_itr], HF_wave[tempTrainingData_itr] = \
                        self.wave_propagate(xsrc, time_index)

                    tempTrainingData_itr = tempTrainingData_itr + 1

        epoch = 0
        train_idxs = list(range(0, datasetSize[0]))
        Epoch = int(args.epoch / self.virtSteps)
        while epoch < Epoch:

            shuffle(train_idxs)
            lr = args.lr if epoch < args.epoch_step else args.lr*(Epoch-epoch)/(Epoch-args.epoch_step)
            print((" [*] Training network #%1d"  % (network_num)))

            for idx in range(0, datasetSize[0]):

                _, summary_str = self.sess.run(
                    [self.g_optim[network_num], self.g_sum[network_num]], \
                    feed_dict={self.Noisy_wave[network_num]: Noisy_wave[train_idxs[idx]], \
                    self.HF_wave: HF_wave[train_idxs[idx]], 
                    self.lr: lr})
                ...
                print(("Epoch: [%2d/%2d] [%4d/%4d] time: %4.4f" % (
                    epoch, Epoch, int(idx), int(datasetSize[0]), time.time() - start_time)))
    ...
    def wave_propagate(self, xsrc, iG):
        ...
        self.solverLF.forward(m=self.model.m, src=self.src, time_m=0, 
            time=(iG+1)*self.virt_timestep,  u=self.u_LF)
        LF_wave = np.transpose(np.array(self.u_LF.data[:, :, :]), \
            (1, 2, 0)).astype(np.float32)[None, :, :, :]
        return LF_wave, HF_wave
    ...
    def sample_model(self, sample_dir, epoch, iG, counter, mask=None):
        ...
        virtStep = choice(list(range(0, self.virtSteps)))        
        Noisy_wave, HF_wave = self.wave_propagate(xsrc, virtStep)
        ...
        feed_dict={self.SNR_diff: diff_img,
                   self.SNR_real: np.transpose(HF_wave[0, :, :, :], \
                    (2, 0 , 1)).reshape((1, self.image_size0 * self.image_size1 * \
                        self.output_c_dim))})
        self.writer.add_summary(summary_str, counter)
        print(("Mapping SNR (Test data) for network #%d: %4.4f" % (iG, snr)))
